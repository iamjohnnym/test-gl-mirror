const branchName =
  process.env.SR_BRANCH_NAME || process.env.CI_COMMIT_REF_NAME || "disabled";
...
const dockerProject =
  process.env.SR_DOCKER_PROJECT || process.env.CI_PROJECT_NAMESPACE || null;
const dockerRegistry =
  process.env.SR_DOCKER_REGISTRY || process.env.CI_REGISTRY || null;
const dockerImage =
  process.env.SR_DOCKER_IMAGE || process.env.CI_PROJECT_NAME || null;
...
const terraformName =
  process.env.SR_TERRAFORM_NAME || process.env.CI_PROJECT_NAME || null;
const terraformDir =
  process.env.SR_TERRAFORM_DIR || process.env.CI_PROJECT_DIR || "./";
const terraformSystem = process.env.SR_TERRAFORM_SYSTEM || "local";
const terraformAuthToken =
  process.env.SR_TERRAFORM_AUTH_TOKEN || process.env.CI_JOB_TOKEN || null;
...
const chartOnlyUpdateVersion =
  process.env.SR_CHART_ONLY_UPDATE_VERSION || false;
...
const defaultOptions = {
  ...
  plugins: {
    ...
    npm: {
      assets: ["package.json", "src/**/*.js", "lib/**/*.js", "docs"],
    },
    gitlab: {},
    git: {
      skipCi: false,
