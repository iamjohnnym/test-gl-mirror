...
function configureDocker(o) {
  ...
  if (o.dockerMulti) {
    if (!docker.dockerTags) {
      const packageJson = openJsonFile(
        `${process.env.npm_workspace_path}/package.json`
      );
      docker.dockerTags = [
  ...
  return [["@codedependant/semantic-release-docker", merge({}, docker)]];
}
