...
const scanProjectDir = async (projectDirPath) =>
  await Promise.all(
    (
      await readdir(projectDirPath, { withFileTypes: true })
    ).map(async (dirent) => {
...
const findFileInProjectContents = (projectFiles, findFilename) => {
  if (
    projectFiles.findIndex((element) => element.includes(findFilename)) !== -1
  ) {
    return true;
  }
  return false;
